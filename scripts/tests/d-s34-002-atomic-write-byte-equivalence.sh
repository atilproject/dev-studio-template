#!/usr/bin/env bash
# d-s34-002-atomic-write-byte-equivalence.sh — Sprint 34 W2 forward-port S34-002 row 002
#
# Verifies scripts/atomic-write.sh in dev-studio-template is byte-equivalent
# to atilcan65/AtilCalculator canonical version. Per ADR-0075 §B.1, this script is
# `equivalent` class (Pure utility, no project context).
#
# Per ADR-0044 RED-first + ADR-0049 ≥6 TCs baseline + ADR-0055 §1 Cadence Rule 1 atomic.
#
# Usage: bash scripts/tests/d-s34-002-atomic-write-byte-equivalence.sh
# Exit: 0 = all TCs GREEN, non-zero = TC failure

set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TARGET_FILE="$SCRIPT_DIR/../atomic-write.sh"
INDEX_FILE="$SCRIPT_DIR/INDEX.md"
CHANGELOG_FILE="$SCRIPT_DIR/../../CHANGELOG.md"

# AtilCalculator canonical MD5 + line count (byte-identical at S34-002 row 002,
# 2026-07-25T07:22Z verified via md5sum + wc -l per cycle ~#495 ground-truth probe)
EXPECTED_MD5="d80f6d0315aaddde6e1cce4f8de97859"
EXPECTED_LINES=75
...
# TC2: file line count = 75 (matches AtilCalculator canonical)
tc2_line_count() {
  local lines
...
# TC4: no project-specific paths (pure utility, no project context per ADR-0075 §B.1)
tc4_no_project_paths() {
  ...
  if [ -z "$grep_result" ]; then
    echo "✅ TC4: no project-specific paths (pure utility confirmed per ADR-0075 §B.1)"
    return 0
  else
...
# TC5: key function signatures present (atomic_write + atomic_write_json per script purpose)
tc5_key_functions() {
  if grep -q "^atomic_write_json()" "$TARGET_FILE" && grep -q "^atomic_write()" "$TARGET_FILE"; then
    echo "✅ TC5: key function signatures present (atomic_write + atomic_write_json)"
    return 0
  else
    echo "❌ TC5: missing key function signatures (atomic_write and/or atomic_write_json)"
    return 1
  fi
}
...
tc6_index_row() {
  if grep -q "d-s34-002-atomic-write-byte-equivalence" "$INDEX_FILE"; then
    echo "✅ TC6: INDEX.md row present (Cadence Rule 1 atomic marker per ADR-0055 §1)"
    return 0
  else
    echo "❌ TC6: INDEX.md row MISSING for d-s34-002-atomic-write-byte-equivalence"
    return 1
  fi
}

# TC7: CHANGELOG.md entry exists for Sprint 34 W2 forward-port S34-002 row 002
tc7_changelog_entry() {
  if grep -q "Sprint 34 W2 forward-port S34-002" "$CHANGELOG_FILE" && grep -q "scripts/atomic-write.sh" "$CHANGELOG_FILE"; then
...
echo ""
if [ "$failed" -eq 0 ]; then
  echo "✅ All 7 TCs GREEN — Sprint 34 W2 forward-port S34-002 row 002 verified"
  echo "   Source-of-truth sister: atilcan65/AtilCalculator/scripts/atomic-write.sh"
