...
# TC6 amender (cycle ~#3968Q+847 inline amender pattern, 2026-07-27T17:22Z): regex tightened to only match actual `removeLabel('...')` / `removeLabel("...")` API calls, not comments/logs that merely mention `remove needs-tester-signoff` wording. False-positives on lines 226 + 629 of canonical workflow confirmed before fix.
cascade_touches_cc=$(grep -E "removeLabel\(['\"]cc:" "$TARGET_FILE" 2>/dev/null | wc -l | tr -d ' ')
cascade_touches_needs=$(grep -E "removeLabel\(['\"]needs-" "$TARGET_FILE" 2>/dev/null | wc -l | tr -d ' ')
...
tc_num=$((tc_num + 1))
# TC7 amender (cycle ~#3968Q+847 inline amender pattern, 2026-07-27T17:22Z): added -i flag for case-insensitive grep — canonical workflow line 17 has "Owner-override:" (capital O) which case-sensitive grep failed to match. Owner-override clause IS present in canonical, false-positive confirmed before fix.
owner_override=$(grep -iE "if:.*github.actor.*==.*atilcan65|owner-override|OWNER_OVERRIDE|owner_override" "$TARGET_FILE" 2>/dev/null | wc -l | tr -d ' ')
