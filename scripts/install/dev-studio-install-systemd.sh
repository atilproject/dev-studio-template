...
for role in "${ROLES[@]}"; do
  ...
  # Per Issue #261: post-restart-label-guard.sh runs after every watcher
  # restart on the developer role only (developer is the only role that
  # owns PRs in the agent-watch labels taxonomy). The script is idempotent
  # (no-op when labels are correct), so redundant invocation is safe; we
  # restrict it to developer for efficiency + to match AtilCalc behavior.
  post_restart_guard=""
  if [ "$role" = "developer" ] && [ -x "$REPO_ROOT/scripts/post-restart-label-guard.sh" ]; then
    post_restart_guard="ExecStartPost=/usr/bin/bash $REPO_ROOT/scripts/post-restart-label-guard.sh"
  fi
...
if [ -x "$REPO_ROOT/scripts/post-restart-label-guard.sh" ]; then
  ok "developer drop-in wired with ExecStartPost= post-restart-label-guard.sh (Issue #261)"
fi
