...
### Added — Sprint 35 S35-003 cluster 2 label-cleanup TD-067 forward-port

- **d-test (NEW, ~190 LOC, 10 TCs)**: `scripts/tests/d-s35-003-c2-label-cleanup.sh` — verifies `label-cleanup.yml` forward-port from bare 108-line tmpl → AtilCalculator's post-Sprint 33 132-line canonical per ADR-0075 §B.1 `label-cleanup.yml` divergent class.
- **Forward-port delta**: TD-067 (Issue #922) TRANSIENT_REGEX scope-tightening — bare `^(cc:|agent:|needs-)|^agent-stall$` → canonical `^(needs-)|^agent-stall$`. Preserves `cc:*` + `agent:*` labels on closed PRs (required by ADR-0012 4-cat invariant + ADR-0009 §10.3 pr_labeled wake audit trail + ADR-0038 auto-claim queue routing + RETRO-016 sister-pattern lineage).
- **10 TCs RED-first per ADR-0044**: TC1 file exists, TC2 YAML syntactic (python yaml.safe_load), TC3 TRANSIENT_REGEX does NOT match cc:tester (TD-067 Part 1), TC4 TRANSIENT_REGEX does NOT match agent:developer (TD-067 Part 2), TC5 TRANSIENT_REGEX DOES match needs-tester-signoff (preserves workflow transient removal), TC6 TRANSIENT_REGEX DOES match agent-stall (preserves stall detection), TC7 TD-067 explanatory header comment present (Issue #922 + design contract anchor), TC8 STATUS_ADVANCE_REGEX preserved (5 transitions covered), TC9 INDEX.md row present, TC10 CHANGELOG.md entry present (unique `Sprint 35 S35-003 cluster 2 label-cleanup` prefix).
- **Pre-port RED VERIFIED @ 2026-07-27T20:45:50Z**: 5 PASS (TC1+TC2+TC5+TC6+TC8) + 5 FAIL (TC3+TC4+TC7+TC9+TC10), exit code 1 NON-VACUOUS.
- **≥6 TCs baseline per ADR-0049**: 10 TCs exceeds baseline by 4. ≥3 sister-pattern coverage met (5 sisters: d-s35-003-c1 + d-s35-003-c2-status-label-to-board + d-s34-005 + d-s34-004 + d096).
- **4-file atomic per ADR-0055 §1**: impl MODIFIED (108-line bare → 132-line canonical + TD-067 tightening + Issue #922 header) + d-test NEW (10 TCs) + INDEX.md MODIFIED + CHANGELOG.md MODIFIED = 4 files same commit cluster per Cadence Rule 1 atomic.
- **Doctrinal anchors**: TD-067 (Issue #922 TRANSIENT_REGEX scope-tightening), ADR-0012 (4-cat label invariant on closed PRs), ADR-0009 §10.3 (pr_labeled wake audit trail), ADR-0038 (auto-claim queue routing), RETRO-016 (TD-067/TD-068 sister-pattern lineage), ADR-0044 (RED-first TDD), ADR-0049 (d-test ≥6 baseline), ADR-0055 §1 (Cadence Rule 1 atomic), ADR-0057 (Refs anchor sub-deliverable pattern), ADR-0031 (owner squash gate), ADR-0033 (dual-channel peer-poke), ADR-0045 (9-Lens coverage), ADR-0059 (cluster-squash ≤5 PRs/cluster), ADR-0075 §B.1 (parity matrix divergent class), cycle ~#3968Q+847 (inline d-test amender — wc -l pattern), cycle ~#3968Q+226 (productive idleness STANDBY), Issue #682 (post-verdict cross-watchdog), Issue #414 §1 (pre-PR re-query).
- **Lane review chain**: arch (Lane 2 PRIMARY 9-Lens per ADR-0045 on TD-067 scope-tightening correctness + 4-cat invariant preservation) + tester (Lane 3 d-test-only sign-off per cycle ~#3642H on d-s35-003-c2-label-cleanup 10 TCs) + owner @atilcan65 (squash gate per ADR-0031).
- **PR body contract** (per ADR-0057): `Refs atilcan65/AtilCalculator#1238` per sub-deliverable pattern (Issue #1238 stays OPEN until Sprint 35 W4 owner-ratification close per RETRO-024, terminal `Closes` reserved for W4 final when ALL clusters 2-5 land).
- **Tester peer-poke pattern**: d-test RED VERIFIED signal sent to dev per ADR-0044 (tester BEFORE dev impl per cycle ~#3893Q v2).

### Added — Sprint 35 S35-003 cluster 2 lint-and-test d031/dreg → d058/d064 swap forward-port

- **d-test (NEW, ~210 LOC, 10 TCs)**: `scripts/tests/d-s35-003-c2-lint-and-test.sh` — verifies `lint-and-test.yml` forward-port from bare 119-line tmpl → AtilCalculator's post-Sprint 33 131-line canonical per ADR-0075 §B.1 `lint-and-test.yml` divergent class.
- **Forward-port delta**: d031/dreg → d058/d064 swap — bare jobs use `d031-claim-next-ready.sh` (10 TCs, ADR-0038 §Layer 2 baseline) + `dreg-post-restart-label-guard.sh` (10 TCs, post-restart hygiene sister) as template-appropriate substitutes; canonical jobs use `d058-claim-wip-workstream.sh` (Sprint 14 P1 #6 AC5, Issue #508, Closes #497 AC5) + `d064-cluster-lag.sh` (Sprint 18 P1, Issue #611, refs STORY-S18-008). Bare header preserves d058/d064 references in adaptation rationale comment (TC3+TC4 PASS today); jobs use d031/dreg substitutes (TC5+TC6 FAIL today).
- **10 TCs RED-first per ADR-0044**: TC1 file exists, TC2 YAML syntactic (python yaml.safe_load), TC3 d058 reference present (canonical Sprint 14 P1 #6 AC5 — PASS via header comment), TC4 d064 reference present (canonical Sprint 18 P1 — PASS via header comment), TC5 d031 reference NOT present (template substitute REMOVED — FAIL today, 2 occurrences in jobs), TC6 dreg reference NOT present (template substitute REMOVED — FAIL today, 2 occurrences in jobs), TC7 actions/checkout SHA-pinned (ADR-0027 + Issue #567, currently PASS), TC8 9-Lens coverage header (ADR-0045 lens a-i annotations, currently PASS), TC9 INDEX.md row present, TC10 CHANGELOG.md entry present (unique `Sprint 35 S35-003 cluster 2 lint-and-test` prefix).
- **Pre-port RED VERIFIED @ 2026-07-27T20:51:22Z**: 6 PASS (TC1+TC2+TC3+TC4+TC7+TC8) + 4 FAIL (TC5+TC6+TC9+TC10), exit code 1 NON-VACUOUS.
- **≥6 TCs baseline per ADR-0049**: 10 TCs exceeds baseline by 4. ≥3 sister-pattern coverage met (6 sisters: d-s35-003-c1 + d-s35-003-c2-status-label-to-board + d-s35-003-c2-label-cleanup + d-s34-005 + d-s34-004 + d096).
- **4-file atomic per ADR-0055 §1**: impl MODIFIED (119-line bare → 131-line canonical + d031/dreg → d058/d064 job swap + S29-010 adaptation header dropped per canonical pattern) + d-test NEW (10 TCs) + INDEX.md MODIFIED + CHANGELOG.md MODIFIED = 4 files same commit cluster per Cadence Rule 1 atomic.
- **Doctrinal anchors**: Sprint 14 P1 #6 AC5 (Issue #508, d058-claim-wip-workstream.sh, Closes #497 AC5), Sprint 18 P1 (Issue #611, d064-cluster-lag.sh, refs STORY-S18-008), ADR-0044 (RED-first TDD), ADR-0049 (d-test ≥6 baseline), ADR-0055 §1 (Cadence Rule 1 atomic), ADR-0057 (Refs anchor sub-deliverable pattern), ADR-0012 (4-cat label invariant), ADR-0027 (SHA-pinning), ADR-0031 (owner squash gate), ADR-0033 (dual-channel peer-poke), ADR-0045 (9-Lens coverage), ADR-0059 (cluster-squash ≤5 PRs/cluster), ADR-0075 §B.1 (parity matrix divergent class), cycle ~#3968Q+847 (inline d-test amender wc -l pattern), cycle ~#3968Q+226 (productive idleness STANDBY), Issue #682 (post-verdict cross-watchdog), Issue #414 §1 (pre-PR re-query).
- **Lane review chain**: arch (Lane 2 PRIMARY 9-Lens per ADR-0045 on d031/dreg → d058/d064 swap correctness + S29-010 adaptation header alignment) + tester (Lane 3 d-test-only sign-off per cycle ~#3642H on d-s35-003-c2-lint-and-test 10 TCs) + owner @atilcan65 (squash gate per ADR-0031).
- **PR body contract** (per ADR-0057): `Refs atilcan65/AtilCalculator#1238` per sub-deliverable pattern (Issue #1238 stays OPEN until Sprint 35 W4 owner-ratification close per RETRO-024, terminal `Closes` reserved for W4 final when ALL clusters 2-5 land).
- **Tester peer-poke pattern**: d-test RED VERIFIED signal sent to dev per ADR-0044 (tester BEFORE dev impl per cycle ~#3893Q v2).

### Added — Sprint 35 S35-003 cluster 2 deploy DEPLOY-001 v5 doctrinal forward-port

- **d-test (NEW, ~230 LOC, 10 TCs)**: `scripts/tests/d-s35-003-c2-deploy.sh` — verifies `deploy.yml` forward-port from bare 112-line tmpl → AtilCalculator's post-Sprint 33 123-line canonical per ADR-0075 §B.1 `deploy.yml` divergent class.
- **Forward-port delta**: DEPLOY-001 v5 doctrinal anchors expansion — bare header is S29-010 themed impl PR 4/4 (PR-D) with OWNER APPROVAL gate notes + S29-001 4-tuple references + PR #1047 d0cf929 reference; canonical header is DEPLOY-001 v5 with full doctrinal cite (Issue #130 user story, #138 P0 incident, #143 DEPLOY-005, #145 workflow YAML update, #148 RCA-5/6, #152 RCA-7/8 P0 incident, #155 v5 spec, ADR-0030 self-hosted runner, ADR-0027 §Decision.2+3+5, ADR-0030 §Threat model, ADR-0010 prod host, DEPLOY-003 GET /healthz).
- **10 TCs RED-first per ADR-0044**: TC1 file exists, TC2 YAML syntactic (python yaml.safe_load), TC3 DEPLOY-001 v5 doctrinal anchor present (PASS via bare header mention), TC4 S29-010 themed impl PR header REMOVED (FAIL today, 3 occurrences), TC5 Issue #130 reference present (FAIL today), TC6 ADR-0030 reference present (FAIL today), TC7 actions/checkout SHA-pinned (ADR-0027 + Issue #567, currently PASS), TC8 9-Lens coverage header (ADR-0045, currently PASS), TC9 INDEX.md row present, TC10 CHANGELOG.md entry present (unique `Sprint 35 S35-003 cluster 2 deploy` prefix).
- **Pre-port RED VERIFIED @ 2026-07-27T20:53:58Z**: 5 PASS (TC1+TC2+TC3+TC7+TC8) + 5 FAIL (TC4+TC5+TC6+TC9+TC10), exit code 1 NON-VACUOUS.
- **≥6 TCs baseline per ADR-0049**: 10 TCs exceeds baseline by 4. ≥3 sister-pattern coverage met (7 sisters: d-s35-003-c1 + d-s35-003-c2-status-label-to-board + d-s35-003-c2-label-cleanup + d-s35-003-c2-lint-and-test + d-s34-005 + d-s34-004 + d096).
- **4-file atomic per ADR-0055 §1**: impl MODIFIED (112-line bare → 123-line canonical + DEPLOY-001 v5 doctrinal anchors expansion + S29-010 themed notes REMOVED + Issue #130/#138/#155 + ADR-0030 references added) + d-test NEW (10 TCs) + INDEX.md MODIFIED + CHANGELOG.md MODIFIED = 4 files same commit cluster per Cadence Rule 1 atomic.
- **Doctrinal anchors**: Issue #130 (DEPLOY-001 user story + closed), Issue #138 (P0 incident), Issue #143 (DEPLOY-005 self-hosted runner install), Issue #145 (workflow YAML update), Issue #148 (RCA-5/6 closed by v4), Issue #152 (RCA-7/8 P0 incident), Issue #155 (DEPLOY-001 v5 spec — RCA-7 4-layer fix), ADR-0030 (self-hosted runner for LAN deploy — SUPERSEDES ADR-0027 §1), ADR-0027 §Decision.2+3+5 (unchanged), ADR-0030 §Threat model (push-only trigger, gh-actions-runner user hardening, SHA pinning), ADR-0010 (prod host + systemd user-service precedent), DEPLOY-003 (#132, #134 merged) — GET /healthz smoke-test target, ADR-0044 (RED-first TDD), ADR-0049 (d-test ≥6 baseline), ADR-0055 §1 (Cadence Rule 1 atomic), ADR-0057 (Refs anchor sub-deliverable pattern), ADR-0012 (4-cat label invariant), ADR-0027 (SHA-pinning), ADR-0031 (owner squash gate), ADR-0033 (dual-channel peer-poke), ADR-0045 (9-Lens coverage), ADR-0059 (cluster-squash ≤5 PRs/cluster), ADR-0075 §B.1 (parity matrix divergent class), cycle ~#3968Q+847 (inline d-test amender wc -l pattern), cycle ~#3968Q+226 (productive idleness STANDBY), Issue #682 (post-verdict cross-watchdog), Issue #414 §1 (pre-PR re-query).
- **Lane review chain**: arch (Lane 2 PRIMARY 9-Lens per ADR-0045 on DEPLOY-001 v5 doctrinal anchor correctness + S29-010 themed notes removal completeness + ADR-0030 alignment) + tester (Lane 3 d-test-only sign-off per cycle ~#3642H on d-s35-003-c2-deploy 10 TCs) + owner @atilcan65 (squash gate per ADR-0031).
- **PR body contract** (per ADR-0057): `Refs atilcan65/AtilCalculator#1238` per sub-deliverable pattern (Issue #1238 stays OPEN until Sprint 35 W4 owner-ratification close per RETRO-024, terminal `Closes` reserved for W4 final when ALL clusters 2-5 land).
- **Tester peer-poke pattern**: d-test RED VERIFIED signal sent to dev per ADR-0044 (tester BEFORE dev impl per cycle ~#3893Q v2). Cluster 2 d-test authoring COMPLETE ✅ (4/4 sister workflow d-tests: status-label-to-board + label-cleanup + lint-and-test + deploy).
