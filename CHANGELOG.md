...
### Removed — Sprint 35 S35-004 obsolete d-test cleanup (NIT follow-up sister-PR per cycle ~#3968Q+3258 Option B-preferred)

- **d-test removal (S35-004 NIT)**: `scripts/tests/d-s35-004-disposable-bootstrap-test-seed-step.sh` (DELETED, was 6 TCs) — obsolete after PR #229 supersedes the seed-step workflow design. PR #228's fix (separate `Seed new repo with dev-studio-template content` step at step 2.5) was replaced by PR #229's atomic `gh repo create --push --source .` approach, which eliminates the separate seed step entirely. Without this removal, the obsolete d-test would FAIL post-amend (TC3-TC6 — the seed step is no longer present in the workflow). Per tester Lane 3 cmt on PR #229: 'obsolete d-s35-004-disposable-bootstrap-test-seed-step.sh d-test now 4/6 RED post-amend (TC8 in atomic d-test covers regression)'.
- **d-test registry**: `scripts/tests/INDEX.md` d-s35-004-disposable-bootstrap-test-seed-step row REMOVED (the row was added with the obsolete d-test in PR #228 commit; removing both atomically per ADR-0055 §1).
- **Branch**: `dev/s35-004-nit-delete-obsolete-seed-d-test` from `tmpl-official/main 9ff3283c` (post-PR #228 squash, pre-PR #229 squash — NIT follows the main amend PR per cycle ~#3258 Option B-preferred sister-PR pattern).
- **Cross-reference**: This NIT sister-PR (PR #230 TBD) is a follow-up to PR #229 (Issue #1254 3rd-order amend, SQUASHED sha ~e5765e2d verdict chain 2/2 PRESERVED per cycle ~#407 62nd). The atomic d-test `d-s35-004-disposable-bootstrap-test-atomic-create-seed.sh` (NEW in PR #229) supersedes the obsolete seed-step d-test with TC8 regression guard — preventing the old seed step from being re-introduced.
- **Sister-pattern**: cycle ~#3968Q+3258 STANDALONE NIT Option B-preferred (separate sister-PR for NIT, NOT blocking main PR). The new atomic d-test TC8 covers the regression guard, so the NIT can be addressed independently without delaying PR #229 owner squash.
- **Doctrinal anchors**: cycle ~#3968Q+847 inline d-test amender (TC5+TC6 amend + TC7+TC8 NEW in same commit — when in-scope), cycle ~#3968Q+3258 (when NIT is out-of-scope — separate sister-PR is preferred), ADR-0049 (d-test ≥6 baseline — the atomic d-test (8 TCs) replaces the obsolete one), ADR-0055 §1 (Cadence Rule 1 atomic — 3 files same commit cluster for this removal PR).
