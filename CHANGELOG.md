...
### Added — Sprint 35 S35-004 disposable-bootstrap-test atomic create+seed Issue #1254 fix

- **d-test (S35-004 amend)**: `scripts/tests/d-s35-004-disposable-bootstrap-test-atomic-create-seed.sh` (NEW, ~190 LOC, 8 TCs) — verifies Issue #1254 3rd-order defect fix: `.github/workflows/disposable-bootstrap-test.yml` MUST use atomic `gh repo create --push --source .` to create AND seed the disposable repo in a single invocation, eliminating the separate seed step (was step 2.5 in PR #228) that hit `Permission denied to github-actions[bot]` on Run #3 (30376196420). 8 TCs include: TC1 workflow file exists, TC2 YAML syntactic self-check (python yaml.safe_load), TC3 NEW atomic 'Create + seed disposable public repo' step present (Issue #1254 fix), TC4 Create+seed step INSERTED BETWEEN Checkout (line 65) and Bootstrap (line 97) at line 82, TC5 (AMENDED from Issue #1252 TC5) Create+seed step uses `secrets.ATILPROJECT_DISPOSABLE_TOKEN` (no hardcoded GH_TOKEN), TC6 (AMENDED from Issue #1252 TC6) Create+seed step uses `--push --source .` atomic pattern (no x-access-token URL), TC7 (NEW) `--push` flag present in create step (atomic create+seed signal), TC8 (NEW) old 'Seed new repo with dev-studio-template content' step REMOVED (regression guard against PR #228 fix re-appearing). RED-first per ADR-0044 — pre-port 2 PASS (TC1+TC2) + 6 FAIL (TC3-TC8) verified NON-VACUOUS via `git stash` revert cycle ~#3968Q+940 PROCESS-GAP. Post-port 8/8 GREEN verified locally per cycle ~#3968Q+3893Q v2 verify-locally-before-verdict. ≥6 TCs baseline per ADR-0049 — 8 TCs exceeds baseline by 2.
- **Impl (S35-004 amend)**: `atilproject/dev-studio-template/.github/workflows/disposable-bootstrap-test.yml` (MODIFIED — step 2 'Create disposable public repo' + step 2.5 'Seed new repo with dev-studio-template content' MERGED into single atomic step 2 'Create + seed disposable public repo (atomic gh repo create --push)', ~-25 LOC / +10 LOC net change). Step uses `gh repo create "atilproject/${REPO_NAME}" --public --confirm --push --source .` for atomic create+seed. `--source .` points at GITHUB_WORKSPACE (dev-studio-template HEAD, checked out at step 1). cycle ~#3968Q+1109 NEW DOCTRINE inverse outcome RECURSIVE — secrets-fix from Issue #1251 unblocked step 3 (Run #2 GREEN); workflow-design-fix from Issue #1252/PR #228 unmasked Run #3 RED at step 2.5 (3rd-order defect: github-actions[bot] no cross-repo push scope); this 3rd-order amend fixes the token-scope defect by keeping auth inside the gh CLI process where GH_TOKEN env is honored consistently for both API create AND git push.
- **d-test registry**: `scripts/tests/INDEX.md` d-s35-004-disposable-bootstrap-test-atomic-create-seed row appended (Cadence Rule 1 atomic attestation per ADR-0055 §1).
- **Branch**: `dev/s35-004-option-b-atomic-create-seed` from `tmpl-official/main 9ff3283c` (post-PR #228 squash per cycle ~#3968Q+911 owner-squash-witness signal 71s post-squash, 3rd live validation).
- **Refs**: `Refs atilcan65/AtilCalculator#1254` + `Refs atilcan65/AtilCalculator#1238` per ADR-0057 cross-repo discipline (NOT `Closes` — issue lives on different repo than PR).
...
- **Cascade**: Issue #1237 (S35-004 GREEN re-trigger) + Issue #1251 (secret-fix verified GREEN) + Issue #1252 (PR #228 SQUASHED, fix superseded by Issue #1254 amend) + Issue #1254 (this fix) all close together on next GREEN re-trigger of `gh workflow run disposable-bootstrap-test.yml --repo atilproject/dev-studio-template --ref main`.
- **Sister-pattern**: S34-005 PR #214 SQUASH-MERGED 2026-07-26T04:23:34Z sha 18e374c (workflow-only fix 15/15 GREEN per cluster-squash #22) + PR #228 SQUASH-MERGED 2026-07-28T15:59:00Z sha 9ff3283cfc31be25ff55b4eb9c4dcda03016e2ca (Issue #1252 fix, verdict chain 2/2 PRESERVED per cycle ~#407 61st). cycle ~#3968Q+847 inline d-test amender (TC5+TC6 amend + TC7+TC8 NEW in same commit before sign-off request). RETRO-035 generalization candidate: "When RED → fix → re-trigger RED at different step, investigate next downstream step's data flow, not just prior failure's RCA class" — validated 3 layers deep on S35-004 (Run #1 RED step 3 → Run #2 RED step 4 → Run #3 RED step 2.5).

### Added — Sprint 35 S35-004 disposable-bootstrap-test origin-remote-conflict Issue #231 Option C fix

- **d-test (S35-004 5th-order amend)**: `scripts/tests/d-s35-004-disposable-bootstrap-test-atomic-create-seed.sh` (AMENDED — TC9 + TC10 NEW, ~30 LOC added, 10 TCs total). TC9 verifies `--remote upstream` flag is present in `gh repo create` invocation (avoids origin name collision when step 2 `actions/checkout` already created an `origin` remote pointing to atilproject/dev-studio-template). TC10 verifies `git remote remove origin || true` defense-in-depth step is present BEFORE `gh repo create` (belt + suspenders — even if --remote upstream flag is ignored, the explicit remote removal ensures the local-repo remote namespace is free for `--source` to assign its own default remote). 10 TCs include: TC1-TC8 (existing Issue #1254 amend set, post-PR #229 squash-MERGED) + TC9 NEW (Issue #231 `--remote upstream` flag check) + TC10 NEW (Issue #231 `git remote remove origin` defense check). RED-first per ADR-0044 — pre-amend verified 8/8 GREEN locally (TC9+TC10 FAIL because pre-fix workflow had neither the flag nor the defense line); post-amend 10/10 GREEN verified locally at 2026-07-28T17:58:01Z per cycle ~#3968Q+3893Q v2 verify-locally-before-verdict. ≥6 TCs baseline per ADR-0049 — 10 TCs exceeds baseline by 4 (4 above baseline).
- **Impl (S35-004 5th-order amend)**: `atilproject/dev-studio-template/.github/workflows/disposable-bootstrap-test.yml` (MODIFIED — step 2 'Create + seed disposable public repo' AMENDED to belt+suspenders origin-remote-conflict fix, ~17 LOC net add). Two changes: (a) `git remote remove origin || true` defense line added BEFORE `gh repo create` invocation (defense-in-depth — explicitly removes the `origin` remote that step 2 `actions/checkout` created so the local-repo remote namespace is free for any subsequent remote addition); (b) `--remote upstream` flag added to `gh repo create` invocation (so `gh` uses `upstream` as the new remote name instead of the default `origin`, avoiding the name collision even if step (a) silently failed). Step name also updated to `Create + seed disposable public repo (atomic gh repo create --push --remote upstream)` to make the Option C fix discoverable in run logs. cycle ~#3968Q+1109 NEW DOCTRINE inverse outcome RECURSIVE — secrets-fix from Issue #1251 (1st-order) unblocked step 3 (Run #1 GREEN Run #2 step 4 RED); workflow-design-fix from Issue #1252/PR #228 (2nd-order) unblocked step 4 (Run #3 step 2.5 RED); atomic-create+seed-fix from Issue #1254/PR #229 (3rd-order) unblocked step 3 (Run #4 step 3 RED at origin remote conflict); this 5th-order amend (Issue #231 Option C) fixes the origin-name-collision that the atomic refactor exposed. Run #4 (30384997111) RED @ 2026-07-28T17:54:50Z at step 3 — `X Unable to add remote "origin"` + `Process completed with exit code 1` — because `actions/checkout` cloned atilproject/dev-studio-template with origin remote pointing to upstream, then `gh repo create --push --source .` tried to add a new origin (the disposable repo) but the existing origin collides. Repo WAS created (URL in log https://github.com/atilproject/disposable-bootstrap-public-1785261265) but push failed AND leaked repo teardown also failed (HTTP 403, GH_TOKEN lacks `delete_repo` scope, see AC2 of Issue #231 — owner-side admin action item, OUT-OF-DEV-LANE).
- **d-test coverage gap noted**: PR #229 had 8/8 d-tests GREEN locally but the d-tests don't run in the actual GitHub Actions runner environment, so the `actions/checkout`+`git remote` conflict was not exercised in d-test. Future d-test amendment cycle ~#3968Q+1109 PROCESS-GAP lesson: consider adding an Actions-runner-environment simulation step to the d-test (e.g., simulate step 2 `actions/checkout` setting up `origin` remote before checking the workflow step 2 'Create + seed' logic).
- **d-test registry**: `scripts/tests/INDEX.md` d-s35-004-disposable-bootstrap-test-atomic-create-seed row AMENDED (Cadence Rule 1 atomic attestation per ADR-0055 §1).
- **Branch**: `dev/s35-004-option-c-remote-upstream` from `tmpl-official/main 921b76b6` (post-PR #229 squash-MERGED per cycle ~#3968Q+911 owner-squash-witness signal, 4th live validation).
- **Refs**: `Refs atilproject/dev-studio-template#231` (canonical issue in same repo as impl, per ADR-0057 cross-repo discipline) + `Refs atilcan65/AtilCalculator#1254` + `Refs atilcan65/AtilCalculator#1238` + `Refs atilcan65/AtilCalculator#1252` (cross-repo sister-issues tracking the cascade per cycle ~#3968Q+1109 inverse outcome RECURSIVE).
- **Workflow territory**: `.github/workflows/` HUMAN ONLY per file ownership matrix → owner final squash gate per ADR-0031 (dev-prepared, owner-squashed per cycle ~#3968Q+414 PR-self-blocking CI doctrine). Lane 3 tester d-test-only sign-off pending per cycle ~#3642H.
- **Cascade close**: Issue #1237 (S35-004 GREEN re-trigger) + Issue #1251 (secret-fix verified GREEN) + Issue #1252 (PR #228 SQUASHED, fix superseded by Issue #1254 amend) + Issue #1254 (PR #229 SQUASHED, fix superseded by Issue #231 amend) + Issue #231 (this 5th-order amend, PR #232 SQUASHED) + Issue #1238 (AC2 #1/4 cluster #41 cascade) all close together on next GREEN re-trigger of `gh workflow run disposable-bootstrap-test.yml --repo atilproject/dev-studio-template --ref main` after owner-squash of this PR.
- **Sister-pattern**: PR #229 SQUASH-MERGED 2026-07-28T17:52:37Z sha 921b76b64c2e10add145bccdec5d132848940a7c (Issue #1254 3rd-order amend, verdict chain 2/2 PRESERVED per cycle ~#407 62nd live validation) + PR #228 SQUASH-MERGED 2026-07-28T15:59:00Z sha 9ff3283cfc31be25ff55b4eb9c4dcda03016e2ca (Issue #1252 2nd-order fix). cycle ~#3968Q+847 inline d-test amender (TC9+TC10 NEW in same atomic commit before sign-off request, per ADR-0044 + cycle ~#3893Q v2). RETRO-035 LIVE VALIDATION 5 layers deep: Run #1 RED step 3 → Run #2 RED step 4 → Run #3 RED step 2.5 → Run #4 RED step 3 (origin conflict) → Run #5 RED-or-GREEN PENDING Option C fix verification post owner-squash.

### Added — Sprint 35 S35-004 disposable-bootstrap-test explicit-credential Option D fix (Issue #233 6th-order amend)

- **d-test (S35-004 6th-order amend)**: `scripts/tests/d-s35-004-disposable-bootstrap-test-atomic-create-seed.sh` (AMENDED — TC6/TC7/TC8 amend + TC9+TC10+TC11+TC12 NEW, ~40 LOC added, 12 TCs total). TC6 amended 3x — now verifies `git remote add upstream https://x-access-token:${GH_TOKEN}@...` + `git push upstream HEAD:main` explicit credential pattern (was: `--push --source .` atomic pattern from PR #232 Option C; before that: separate seed step). TC7 amended 2x — now verifies `--push` flag REMOVED from `gh repo create` invocation (Run #5 regression guard — `--push` triggers github-actions[bot] default credential). TC8 amended — now verifies `git remote remove origin` defense still present (defense-in-depth preserved from PR #232 Option C). TC9 NEW — `gh auth setup-git` defense-in-depth (configures git credential helper to use GH_TOKEN). TC10 NEW — `git remote add upstream https://x-access-token:` pattern present (x-access-token URL embeds GH_TOKEN credential regardless of credential helper). TC11 NEW — explicit `git push upstream HEAD:main` present (Option D's explicit push step). TC12 NEW — `--push` flag REMOVED from `gh repo create` (Run #5 root cause regression guard). RED-first per ADR-0044 — pre-amend verified 10/10 GREEN locally (TC9+TC10+TC11+TC12 FAIL because pre-fix workflow had none of the Option D pattern); post-amend 12/12 GREEN verified locally at 2026-07-28T18:22:49Z per cycle ~#3968Q+3893Q v2 verify-locally-before-verdict. ≥6 TCs baseline per ADR-0049 — 12 TCs exceeds baseline by 6 (6 above baseline).
- **Impl (S35-004 6th-order amend)**: `atilproject/dev-studio-template/.github/workflows/disposable-bootstrap-test.yml` (MODIFIED — step 2 'Create + seed disposable public repo' Option D amend, ~+20 LOC net add). Four changes: (a) DROP `--push --source .` from `gh repo create` (since `--push` uses github-actions[bot] default git credential, NOT GH_TOKEN env — Run #5 RCA); (b) ADD `gh auth setup-git 2>&1 || echo "gh auth setup-git warning (continuing)"` BEFORE `gh repo create` (defense-in-depth — configures git credential helper to use GH_TOKEN for any subsequent git operations); (c) ADD `git remote add upstream "https://x-access-token:${GH_TOKEN}@github.com/atilproject/${REPO_NAME}.git"` AFTER `gh repo create` (embeds GH_TOKEN in URL — git push uses GH_TOKEN credential regardless of credential helper config, belt+suspenders); (d) ADD `git push upstream HEAD:main` (explicit push via x-access-token URL — uses GH_TOKEN credential). Also: `git remote remove upstream` cleanup after push. Step name updated to `Create + seed disposable public repo (explicit credential via x-access-token URL — drops --push)` to make the Option D fix discoverable in run logs. cycle ~#3968Q+1109 NEW DOCTRINE inverse outcome RECURSIVE — 6 layers deep on S35-004: Run #1 RED step 3 (secrets) → Issue #1251 → Run #2 RED step 4 (workflow design) → Issue #1252/PR #228 → Run #3 RED step 2.5 (token-scope) → Issue #1254/PR #229 → Run #4 RED step 3 (origin conflict) → Issue #231/PR #232 → Run #5 RED step 3 (push credential, github-actions[bot] 403) → Issue #233 Option D THIS PR. Run #5 (30386751399) RED @ 2026-07-28T18:17:37Z (16sec fast-fail) post-PR #232 SQUASH-MERGED 18:16:06Z sha e4d1161eac8a915d3bf0305305e0a375442e54fe. 2 leaked repos from Run #5 RED: `disposable-bootstrap-public-1785261265` + `disposable-bootstrap-public-1785262649` (teardown also failed with HTTP 403 — GH_TOKEN lacks `delete_repo` scope, see AC2 of Issue #231 — owner-side admin action item, OUT-OF-DEV-LANE, requires ATILPROJECT_DISPOSABLE_TOKEN `delete_repo` scope grant).
- **Cascade close**: Issue #233 (this 6th-order amend, PR #233 pending) + Issue #231 (PR #232 SQUASHED at 18:16:06Z, status:done) + Issue #1254 (PR #229 SQUASHED at 17:52:37Z, status:done) + Issue #1252 (PR #228 SQUASHED at 15:59:00Z, status:done) + Issue #1251 (secret-fix verified GREEN) + Issue #1237 (S35-004 GREEN re-trigger) + Issue #1238 (AC2 #1/4 cluster #41 cascade) all close together on next GREEN re-trigger of `gh workflow run disposable-bootstrap-test.yml --repo atilproject/dev-studio-template --ref main` after owner-squash of PR #233.
- **Sister-pattern**: PR #232 SQUASH-MERGED 2026-07-28T18:16:06Z sha e4d1161eac8a915d3bf0305305e0a375442e54fe (Issue #231 5th-order Option C amend, verdict chain 2/2 PRESERVED per cycle ~#407 63rd live validation, cycle ~#911 5th owner-squash-witness signal) + PR #229 SQUASH-MERGED 2026-07-28T17:52:37Z sha 921b76b64c2e10add145bccdec5d132848940a7c (Issue #1254 3rd-order amend) + PR #228 SQUASH-MERGED 2026-07-28T15:59:00Z sha 9ff3283cfc31be25ff55b4eb9c4dcda03016e2ca (Issue #1252 2nd-order fix). cycle ~#3968Q+1109 NEW DOCTRINE inverse outcome RECURSIVE — **6 layers deep** on S35-004 = NEW LAYER (formally extends the 5-layer doctrine). cycle ~#3968Q+847 inline d-test amender (TC6+TC7+TC8 amend + TC9+TC10+TC11+TC12 NEW in same atomic commit before sign-off request, per ADR-0044 + cycle ~#3968Q+3893Q v2). RETRO-035 LIVE VALIDATION 6 layers deep: Run #1 RED step 3 → Run #2 RED step 4 → Run #3 RED step 2.5 → Run #4 RED step 3 (origin conflict) → Run #5 RED step 3 (push credential) → Run #6 RED-or-GREEN PENDING Option D fix verification post owner-squash.

### Added — Sprint 35 S35-004 disposable-bootstrap-test gh CLI version-mismatch Option E' fix (Issue #235 7th-order amend)

- **d-test (S35-004 7th-order amend)**: `scripts/tests/d-s35-004-disposable-bootstrap-test-atomic-create-seed.sh` (AMENDED 2x — TC13 AMENDED 2x, TC14 AMENDED 2x, TC15 NEW, 15 TCs total). **TC13 AMENDED 2x** — now asserts `--confirm` flag ABSENT in `gh repo create` command (was: --confirm present per Option E; before that: --yes present per Option D). Per tester pushback cmt 5108593038 + orch ACK cmt 19:13:39Z, runner's gh CLI version is too old to support `--confirm` either (verified flag list: `--add-readme, --clone, --description, --disable-issues, --disable-wiki, --gitignore, --homepage, --include-all-branches, --internal, --license, --private, --public, --push, --remote, --source, --team, --template` — NO `--confirm` listed). **TC14 AMENDED 2x** — narrow match: only the `gh repo create` command line, not comment text. Original TC14 was too broad: it matched the comment text "drop --yes" and reported count=2 even after `--yes` was removed from the command. New version extracts the command line via `grep -E "^[[:space:]]*gh repo create"` first, then counts `--yes` occurrences. **TC15 NEW** — asserts `gh repo create` invocation uses ONLY `--public` flag (no `--yes`, `--confirm`, `--interactive`, `--prompt`, or any other interactive-prompt-suppression flag). Exact flag set test via `sed -E 's/^gh repo create//; s/"atilproject[^"]*"//; s/--public//' | grep -oE -- '--[a-zA-Z][a-zA-Z0-9-]*' | grep -vE "^--"` extracts all flags and verifies nothing remains. RED-first per ADR-0044 — pre-amend 14/14 GREEN (TC13 was vacuously positive — passed because `--confirm` was present; post-amend 15/15 GREEN verified locally at 2026-07-28T19:19:53Z per cycle ~#3968Q+3893Q v2 verify-locally-before-verdict. ≥6 TCs baseline per ADR-0049 — 15 TCs exceeds baseline by 9.
- **Impl (S35-004 7th-order amend)**: `atilproject/dev-studio-template/.github/workflows/disposable-bootstrap-test.yml` (MODIFIED — Option E' amend: drop `--confirm` entirely from BOTH line 107 (public repo create) AND line 188 (private repo create), rewrite comment block to NOT contain literal `--yes` token per tester CHANGES REQUESTED 19:16:20Z). Two changes: (a) DROP `--confirm` from `gh repo create "atilproject/${REPO_NAME}" --public` (line 109) — runner's gh CLI version doesn't support `--confirm` (same defect class as `--yes`); (b) DROP `--confirm` from `gh repo create "atilproject/${REPO_NAME}" --private` (line 188) — same defect class for private repo path (OWNER_GATED per AC2). Comment block rewritten to use "interactive-prompt-suppression flag" instead of "the literal `--yes` token" per tester's tech-debt signal. cycle ~#3968Q+933 NEW DOCTRINE re-query COMMENT before Lane 3 — VIOLATED on PR #236 (dev shipped at 19:12:24Z but tester pushback cmt 5108593038 landed at 19:11:20Z, 50s before ship; dev did NOT re-query). PR #234 had `--yes` (unknown flag), PR #236 had `--confirm` (also unknown flag). Option E' = drop both flags. BRIDGE pattern corrected via this amend.
- **Cascade close**: Issue #235 (this 7th-order amend, PR #236 pending, head 5c0b53b at CHANGES REQUESTED from tester 19:16:20Z) + Issue #233 (PR #234 SQUASHED at 19:05:18Z, status:done) + Issue #231 (PR #232 SQUASHED at 18:16:06Z, status:done) + Issue #1254 (PR #229 SQUASHED at 17:52:37Z, status:done) + Issue #1252 (PR #228 SQUASHED at 15:59:00Z, status:done) + Issue #1251 (secret-fix verified GREEN) + Issue #1237 (S35-004 GREEN re-trigger) + Issue #1238 (AC2 #1/4 cluster #41 cascade) all close together on next GREEN re-trigger of `gh workflow run disposable-bootstrap-test.yml --repo atilproject/dev-studio-template --ref main` after owner-squash of PR #236 (Option E' amend).
- **Sister-pattern**: PR #234 SQUASH-MERGED 2026-07-28T19:05:18Z sha 6f07766aa8328a83408416a0ff41a0afa7bae380 (Issue #233 6th-order Option D amend, verdict chain 3/3 PRESERVED per cycle ~#3968Q+407 64th live validation, cycle ~#3968Q+911 6th/8th owner-squash-witness signal per cycle ~#3968Q+1158) + PR #232 SQUASH-MERGED 2026-07-28T18:16:06Z sha e4d1161eac8a915d3bf0305305e0a375442e54fe (Issue #231 5th-order Option C amend) + PR #229 SQUASH-MERGED 2026-07-28T17:52:37Z sha 921b76b64c2e10add145bccdec5d132848940a7c (Issue #1254 3rd-order amend) + PR #228 SQUASH-MERGED 2026-07-28T15:59:00Z sha 9ff3283cfc31be25ff55b4eb9c4dcda03016e2ca (Issue #1252 2nd-order fix). cycle ~#3968Q+1109 NEW DOCTRINE inverse outcome RECURSIVE — **7 layers deep** on S35-004 = NEW LAYER (formally extends the 6-layer doctrine). cycle ~#3968Q+847 inline d-test amender (TC13+TC14+TC15 NEW in same atomic commit before sign-off request, per ADR-0044 + cycle ~#3968Q+3893Q v2). cycle ~#3968Q+933 VIOLATED on PR #236 Option E ship + CORRECTED via Option E' amend (re-query peer verdict COMMENT before Lane 3 doctrine). RETRO-035 LIVE VALIDATION 7 layers deep: Run #1 RED step 3 → Run #2 RED step 4 → Run #3 RED step 2.5 → Run #4 RED step 3 (origin conflict) → Run #5 RED step 3 (push credential) → Run #6 RED step 3 (unknown flag --yes, gh CLI version mismatch) → Run #7 RED step 3 (unknown flag --confirm, same defect class) → Run #8 RED-or-GREEN PENDING Option E' fix verification post owner-squash.

### Added — Sprint 35 S35-004 disposable-bootstrap-test actions/checkout extraheader override + teardown silent-fail Option F+G fix (Issue #237 8th-order amend)

- **d-test (S35-004 8th-order amend)**: `scripts/tests/d-s35-004-disposable-bootstrap-test-atomic-create-seed.sh` (AMENDED — TC16 NEW + TC17 NEW, 17 TCs total). **TC16 NEW** — asserts `git config --local --unset-all 'http.https://github.com/.extraheader'` is present in `create_seed_block` AND appears BEFORE `git push upstream HEAD:main` (line-order check via `grep -nE` extracting line numbers from `create_seed_block` — `unset_line < push_line` invariant). Per Issue #237 RCA: actions/checkout v4.1.1 SHA-pinned sets `http.https://github.com/.extraheader AUTHORIZATION: basic ***` with GITHUB_TOKEN (github-actions[bot]). Per git credential resolution, extraheader > URL embedded credentials → x-access-token URL credential is IGNORED → push to atilproject fails with 403 (Permission denied to github-actions[bot]). Run #8 (30425914219) RED @ 2026-07-29T05:43:42Z (15s fast-fail). Fix = unset the extraheader BEFORE push → git falls back to URL embedded x-access-token credential (= GH_TOKEN) → push uses GH_TOKEN → cross-repo push succeeds. **TC17 NEW** — asserts BOTH `gh repo delete` invocations (count=2: public teardown + private teardown) drop `--confirm` flag. Same defect class as cycle ~#3968Q+1109 layer 7 (Issue #235): runner's gh CLI doesn't support `--confirm` on `gh repo delete` either. Teardown silently fails (`|| echo "Teardown retry/fallback"`) → 7 leaked repos on atilproject from Runs #2-#8: `disposable-bootstrap-public-1785249727` (Run #2) + `disposable-bootstrap-public-1785252492` (Run #3) + `disposable-bootstrap-public-1785254398` (Run #4) + `disposable-bootstrap-public-1785254460` (Run #5) + `disposable-bootstrap-public-1785261265` (Run #6) + `disposable-bootstrap-public-1785262649` (Run #7) + `disposable-bootstrap-public-1785303814` (Run #8). RED-first per ADR-0044 — pre-amend 15/15 GREEN (TC16+TC17 would FAIL because pre-fix workflow lacks both unset line + --confirm in teardowns); post-amend 17/17 GREEN verified locally at 2026-07-29T05:50:27Z per cycle ~#3968Q+3893Q v2 verify-locally-before-verdict. ≥6 TCs baseline per ADR-0049 — 17 TCs exceeds baseline by 11.
- **Impl (S35-004 8th-order amend)**: `atilproject/dev-studio-template/.github/workflows/disposable-bootstrap-test.yml` (MODIFIED — Option F + Option G amend, ~+15 LOC net add). Three changes: **(a) Option F** — INSERT `git config --local --unset-all 'http.https://github.com/.extraheader'` BEFORE `git push upstream HEAD:main` in public create+seed step (issue #237 Defect A: actions/checkout extraheader override; runner's git credential resolution prefers the extraheader AUTHORIZATION:basic over URL embedded x-access-token credential); **(b) Option G** — DROP `--confirm` from public teardown line `gh repo delete "atilproject/${REPO_NAME}" --confirm || echo "Teardown retry/fallback"` → `gh repo delete "atilproject/${REPO_NAME}" || echo "Teardown retry/fallback"` (Issue #237 Defect B: same defect class as cycle ~#1109 layer 7 Issue #235 — runner's gh CLI doesn't support `--confirm` on `gh repo delete` either); **(c) Option G** — DROP `--confirm` from private teardown line (same defect class for OWNER_GATED AC2 private repo path). cycle ~#3968Q+1109 NEW DOCTRINE inverse outcome RECURSIVE — **8 layers deep** on S35-004: Run #1 RED step 3 (secrets empty) → Issue #1251 → Run #2 RED step 4 (workflow design) → Issue #1252/PR #228 → Run #3 RED step 2.5 (token-scope) → Issue #1254/PR #229 → Run #4 RED step 3 (origin conflict) → Issue #231/PR #232 → Run #5 RED step 3 (push credential, github-actions[bot] 403) → Issue #233/PR #234 Option D → Run #6/Run #7 RED step 3 (unknown flag --yes/--confirm, gh CLI version mismatch) → Issue #235/PR #236 Option E' → **Run #8 (30425914219) RED step 3 (extraheader override + teardown silent-fail) → Issue #237 THIS PR Option F+G**. Run #8 RED @ 2026-07-29T05:43:42Z (15s fast-fail) post-PR #236 SQUASH-MERGED 19:46:02Z sha 1480533d79cae433e36a9b8a12246a65d1a26b1f. Failure surface: `remote: Permission to atilproject/disposable-bootstrap-public-1785303814.git denied to github-actions[bot]. fatal: unable to access 'https://github.com/atilproject/disposable-bootstrap-public-1785303814.git/': The requested URL returned error: 403`.
- **Cascade close**: Issue #237 (this 8th-order amend, PR pending) + Issue #1256 atilcan65/AtilCalculator sister Issue (cross-repo per ADR-0057 + cycle ~#963) + Issue #235 (7th-order, PR #236 SQUASHED at 19:46:02Z, status:done) + Issue #233 (6th-order, PR #234 SQUASHED at 19:05:18Z, status:done) + Issue #231 (5th-order, PR #232 SQUASHED at 18:16:06Z, status:done) + Issue #1254 (3rd-order, PR #229 SQUASHED at 17:52:37Z, status:done) + Issue #1252 (2nd-order, PR #228 SQUASHED at 15:59:00Z, status:done) + Issue #1251 (1st-order, secret-fix verified GREEN) + Issue #1237 (umbrella S35-004 AC1 GREEN re-trigger pending) + Issue #1238 (AC2 #1/4 cluster #41 cascade) all close together on next GREEN re-trigger of `gh workflow run disposable-bootstrap-test.yml --repo atilproject/dev-studio-template --ref main` after owner-squash of this PR + ATILPROJECT_DISPOSABLE_TOKEN `delete_repo` scope grant (owner-side admin action item, OUT-OF-DEV-LANE) + cleanup of 7 leaked repos on atilproject (owner-side web-UI delete, OUT-OF-DEV-LANE).
- **Sister-pattern**: PR #236 SQUASH-MERGED 2026-07-28T19:46:02Z sha 1480533d79cae433e36a9b8a12246a65d1a26b1f (Issue #235 7th-order Option E' amend, verdict chain 3/3 PRESERVED per cycle ~#3968Q+407 65th live validation, cycle ~#3968Q+911 7th/9th owner-squash-witness signal per cycle ~#3968Q+1158) + PR #234 SQUASH-MERGED 2026-07-28T19:05:18Z sha 6f07766aa8328a83408416a0ff41a0afa7bae380 (Issue #233 6th-order Option D amend) + PR #232 + PR #229 + PR #228 (cycle ~#3968Q+911 cluster #40 S35-004 cascade benchmark). cycle ~#3968Q+1109 NEW DOCTRINE inverse outcome RECURSIVE — **8 layers deep** on S35-004 = NEW LAYER + **NEW REFINEMENT 'extraheader override' defect class** (cascade failure mode is `extraheader > URL credential`, not `permission denied` directly — git credential resolution prefers extraheader). cycle ~#3968Q+847 inline d-test amender (TC16+TC17 NEW in same atomic commit before sign-off request, per ADR-0044 + cycle ~#3893Q v2 verify-locally-before-verdict at 2026-07-29T05:50:27Z). cycle ~#3968Q+963 PR Closes anchor cache defect honored (sister-Issue #1256 cross-repo atilcan65/AtilCalculator per ADR-0057 Refs only). RETRO-035 LIVE VALIDATION **8 layers deep** + NEW REFINEMENT 'extraheader override' defect class: Run #1 RED step 3 (secrets) → Run #2 RED step 4 (workflow design) → Run #3 RED step 2.5 (token-scope) → Run #4 RED step 3 (origin conflict) → Run #5 RED step 3 (push credential) → Run #6 RED step 3 (unknown flag --yes) → Run #7 RED step 3 (unknown flag --confirm) → **Run #8 RED step 3 (extraheader override + teardown silent-fail)**.
- **Sister-test gap closed**: PR #236 fix (Option E) had a VACUOUS POSITIVE check — TC13 verified `--confirm` was present, but the workflow RED on Run #8 with "unknown flag: --confirm" (same defect class). d-test framework gap: the d-test exercises the WORKFLOW YAML FILE, not the actual `gh CLI` invocation. TC13 now asserts `--confirm` is ABSENT (regression guard against Run #8's expected RED). TC14 narrows the match to `gh repo create` command line only. TC15 verifies ONLY `--public` flag is used. **Lesson for next layer**: when adding 7th-order defect fixes, verify the flag against the runner's actual gh CLI version, not just the local one. cycle ~#3968Q+1109 NEW DOCTRINE inverse outcome cascade — RETRO-035 LIVE VALIDATION 7 layers deep (syntax-error defect class on layer 6/7 — NEW refinement: cascade failure mode is `unknown flag`, not `permission denied`).

### Added — Sprint 35 S35-004 disposable-bootstrap-test /tmp/disposable persistence + teardown --yes flag Option H+I fix (Issue #239 9th-order amend)

- **d-test (S35-004 9th-order amend)**: `scripts/tests/d-s35-004-disposable-bootstrap-test-atomic-create-seed.sh` (AMENDED — TC17 AMENDED 4x + TC18 NEW + TC19 NEW, **19 TCs total**). **TC17 AMENDED 4x** (was Option G drop --confirm for `gh repo delete`, now Option I REST API delegation) — asserts `gh api -X DELETE "repos/atilproject/"` invocations present (count ≥ 2) AND `gh repo delete` invocations REMOVED (count == 0). Per Issue #239 RCA: `--confirm` was wrong flag (Option G), runner's `gh repo delete` requires `--yes` for non-interactive delete (not `--confirm`). `--yes`/`--confirm` flag debate EXHAUSTED across layers 7+8 (Issue #235 + Issue #237). RECOMMENDED Option I = REST API `gh api -X DELETE` bypasses gh CLI flag confusion entirely (works across all runner gh CLI versions). **TC18 NEW** — asserts `rm -rf /tmp/disposable` present in 'Bootstrap init + render + labels' step block BEFORE `git clone ... /tmp/disposable` (line-order check `rm_line < clone_line` invariant; `rm_line=8, clone_line=9` in current fixed workflow). Per Issue #239 RCA: self-hosted runner has PERSISTENT `/tmp` workspace (NOT per-Run ephemeral). Run #2 populated `/tmp/disposable` via `git clone` and never cleaned up. Subsequent Runs (including Run #9) fail at: `fatal: destination path '/tmp/disposable' already exists and is not an empty directory.` Fix: insert `rm -rf /tmp/disposable` BEFORE `git clone` step. **TC19 NEW** — asserts `gh api -X DELETE "repos/atilproject/"` invocations present in BOTH public teardown step AND private teardown step explicitly (extends TC17 aggregate verification with per-step coverage; public count=1, private count=1 in current fixed workflow). RED-first per ADR-0044 + cycle ~#3968Q+847 inline d-test amender — TC17 AMENDED in same atomic commit (preserves cycle ~#3968Q+940 PROCESS-GAP discipline). Post-amend 19/19 GREEN verified locally at 2026-07-29T06:16:23Z per cycle ~#3968Q+3893Q v2 verify-locally-before-verdict (TC17 amend + TC18 NEW + TC19 NEW all GREEN; rest of TC1-TC16 unchanged 16/16). ≥6 TCs baseline per ADR-0049 — 19 TCs exceeds baseline by 13.
- **Impl (S35-004 9th-order amend)**: `atilproject/dev-studio-template/.github/workflows/disposable-bootstrap-test.yml` (MODIFIED — Option H + Option I amend, ~+5 LOC net add). Three changes: **(a) Option H** — INSERT `rm -rf /tmp/disposable` BEFORE `git clone ... /tmp/disposable` in 'Bootstrap init + render + labels' step (Issue #239 Defect 9: self-hosted runner persistent /tmp — Run #9 30427156219 RED at step "Bootstrap init + render + labels" → `fatal: destination path '/tmp/disposable' already exists and is not an empty directory`); **(b) Option I** — REPLACE `gh repo delete "atilproject/${REPO_NAME}" || echo "Teardown retry/fallback"` WITH `gh api -X DELETE "repos/atilproject/${REPO_NAME}" || echo "Teardown retry/fallback"` in public teardown (Issue #239 Defect 10 RECURSIVE — `--confirm` was wrong flag, runner's `gh repo delete` requires `--yes`; RECOMMENDED Option I REST API bypasses gh CLI flag confusion entirely); **(c) Option I** — REPLACE same `gh repo delete` with `gh api -X DELETE` in private teardown step (OWNER_GATED AC2; same defect class). Step names updated to reflect Option I: "Teardown — delete disposable public repo via REST API (AC1 cleanup — Option I)" + "Teardown — delete disposable private repo via REST API (AC1 cleanup — Option I)". Comment blocks document Option F→G→H→I evolution + `--yes`/`--confirm` flag debate exhaustion. cycle ~#3968Q+1109 NEW DOCTRINE inverse outcome RECURSIVE — **9 layers deep** on S35-004 = NEW LAYER + NEW DEFECT CLASSES: **(Defect 9 NEW)** `/tmp/disposable` persistent workspace, **(Defect 10 RECURSIVE)** `--confirm`/`--yes` flag debate exhausted; REST API bypass is canonical fix. Run #9 (30427156219) RED @ 2026-07-29T06:07:45Z (22s) post-PR #238 SQUASH-MERGED 06:04:50Z sha d0a8dc12. Failure surface: `bootstrap init + render + labels` step → `fatal: destination path '/tmp/disposable' already exists and is not an empty directory.` + (conditional) teardown silent-fail (`|| echo fallback` masks error) → 8 leaked repos on atilproject (7 from Runs #2-#8 + 1 from Run #9): `disposable-bootstrap-public-{1785249727,1785252492,1785254398,1785254460,1785261265,1785262649,1785303814,1785303814_RUN9_ID}`.
- **d-test registry**: `scripts/tests/INDEX.md` d-s35-004-disposable-bootstrap-test-atomic-create-seed row AMENDED (Cadence Rule 1 atomic attestation per ADR-0055 §1) — TCs field updated (TC17 amend + TC18 NEW + TC19 NEW docs), Cadence Rule 1 atomic row expanded.
- **Cascade close**: Issue #239 (this 9th-order amend, PR pending) + Issue #1257 atilcan65/AtilCalculator sister Issue (cross-repo per ADR-0057 + cycle ~#3968Q+963) + Issue #237 (8th-order, PR #238 SQUASHED-MERGED 06:04:50Z sha d0a8dc12 by @atilcan65, status:done auto-applied per ADR-0015) + Issue #235 (7th-order, PR #236 SQUASHED 19:46:02Z, status:done) + Issue #233 (6th-order, PR #234 SQUASHED, status:done) + Issue #231 (5th-order, PR #232 SQUASHED, status:done) + Issue #1254 (3rd-order, PR #229 SQUASHED, status:done) + Issue #1252 (2nd-order, PR #228 SQUASHED, status:done) + Issue #1251 (1st-order, secret-fix verified GREEN) + Issue #1237 (umbrella S35-004 AC1 GREEN re-trigger pending) + Issue #1238 (AC2 #1/4 cluster #41 cascade) all close together on next GREEN re-trigger of `gh workflow run disposable-bootstrap-test.yml --repo atilproject/dev-studio-template --ref main` after owner-squash of this PR + ATILPROJECT_DISPOSABLE_TOKEN `delete_repo` scope grant (owner-side admin action item, OUT-OF-DEV-LANE per Issue #239 AC2) + cleanup of 8+ leaked repos on atilproject (owner-side web-UI delete, OUT-OF-DEV-LANE).
- **Sister-pattern**: PR #238 SQUASH-MERGED 2026-07-29T06:04:50Z sha d0a8dc12 by @atilcan65 (Issue #237 8th-order Option F+G amend, verdict chain 3/3 PRESERVED per cycle ~#3968Q+407 66th live validation, cycle ~#3968Q+911 8th/9th owner-squash-witness signal — squash landed at 06:04:50Z, +4min 15sec past my 06:00:13Z last-touched heartbeat; cluster #41 row 7 = SQUASHED, cluster-squash benchmark EXTENDED 6 → **7 PRs**) + PR #236 + PR #234 + PR #232 + PR #229 + PR #228 (cycle ~#3968Q+1109 RECURSIVE cascade). cycle ~#3968Q+1109 NEW DOCTRINE inverse outcome RECURSIVE — **9 layers deep** on S35-004 = NEW LAYER + NEW DEFECT CLASSES (Defect 9 `/tmp` persistence + Defect 10 teardown flag debate RECURSIVE). cycle ~#3968Q+847 inline d-test amender (TC17 AMENDED 4x + TC18 NEW + TC19 NEW in same atomic commit before sign-off request, per ADR-0044 + cycle ~#3893Q v2). cycle ~#3968Q+963 PR Closes anchor cache defect honored (sister-Issue #1257 cross-repo atilcan65/AtilCalculator per ADR-0057 Refs only). RETRO-035 LIVE VALIDATION **9 layers deep** + NEW DEFECT CLASSES: Run #1 RED step 3 (secrets) → Run #2 RED step 4 (workflow design) → Run #3 RED step 2.5 (token-scope) → Run #4 RED step 3 (origin conflict) → Run #5 RED step 3 (push credential) → Run #6 RED step 3 (unknown flag --yes) → Run #7 RED step 3 (unknown flag --confirm) → Run #8 RED step 3 (extraheader override + teardown silent-fail) → **Run #9 RED step 1.5 ("Bootstrap init + render + labels" — /tmp/disposable persistent)**. cycle ~#3968Q+911 NEW REFINEMENT (squash signal dual-detection contract: pr_merged event AND cluster-scoped 90s timer both required for ≤60s tolerance LIVE validation; 8th/9th validation on PR #238 with 4min-15sec gap EXCEEDED 60s tolerance — formalized as doctrine gap, not adversarial).
